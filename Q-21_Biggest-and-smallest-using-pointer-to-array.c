...
/*  1. Taking size of array into 'n'
    ...
    3. Creating base pointer for arr, of name base_ptr; this pointer will always point to first element of array
...
int main()  {
    ...
    double *base_ptr = &arr[0];
    double *moving_ptr;
    double *min_ptr = base_ptr; //Assuming the initial element of array as minimum
    double *max_ptr = base_ptr; //Assuming the initial element of array as maximum

    for (int i=0; i<n; i++)  {
        moving_ptr = &arr[i];
        if (*base_ptr > *moving_ptr)
            min_ptr = moving_ptr;
    }
